...
function onConnectToIRC(socket, data) {
  const { server, port = 6667, initialChannels, nickname } = data;

  // Check for duplicate connections for client.
  var exists = socket.id in clients;
  ...
  var channels = initialChannels.length > 0 ? initialChannels.split(",") : [];
      ...
      if (target && msg) {
        if (msg.indexOf("\u0001ACTION") === 0) {
          ...
          text = msg
            .split("\u0001")[1]
            .split("ACTION")[1]
            .trim(); // Get just the text portion.
  ...
  socket.on("webMessage", function(data) {
    ...
    // Say in IRC.
    if (target !== "main") {
      // The main window will not exist as a channel.
  ...
  socket.on("webCommand", function(data) {
    ...
    var cmdMapping = {
      "/join": {
        func: doJoin,
        helpText:
          "Join a channel, takes a channel argument. ex: /join #somechannel"
      },
      "/j": { func: doJoin, helpText: "Short hand for /join" },
      "/part": {
        func: doPart,
        helpText:
          "Parts the current channel, takes an optional reason argument. ex: /part no more love to give"
      },
      "/p": { func: doPart, helpText: "Short hand for /part" },
      "/topic": {
        func: doTopic,
        helpText:
          "Changes the channel topic, takes an argument. ex: /topic i like unicorns"
      },
      "/t": { func: doTopic, helpText: "Short hand for /topic" },
      "/quit": {
        func: doQuit,
        helpText:
          "Quits. Everything. Takes an optional reason argument. ex: /quit self destruct"
      },
      "/q": { func: doQuit, helpText: "Short hand for /quit" },
      "/nick": {
        func: doNick,
        helpText: "Changes your nick, takes one argument. ex: /nick ZeroCool"
      },
      "/n": { func: doNick, helpText: "Short hand for /nick" },
      "/msg": {
        func: doMsg,
        helpText:
          "Sends a private message to channel or person, takes two arguments target and message. ex: /msg ZeroCool seriously? change your name!"
      },
      "/m": { func: doMsg, helpText: "Short hand for /msg" },
      "/me": {
        func: doAction,
        helpText: "Do an action! Takes an argument. ex: /me slaps ZeroCool"
      },
      "/help": {
        func: doHelp,
        helpText:
